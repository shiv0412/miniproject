    ...
    <div class="header_container">
      ...
      <div class="form_heading_container">
        <center><p class="form_head_p">
          <h2>Check Users Queries</h2>
        </p></center>
      </div>
    </div>
      <div class="search_page_form_con">
       <div class="registration_form_container">
      <form class="row g-3 form_content" action="bank_search_query.php"  method="POST">
...
$result_p=mysql_query("SELECT password FROM login WHERE  password LIKE '$password'");
$row_p=mysql_fetch_array($result_p);
$check_pass=$row_p['password'];
if ($check_pass == $password )
{
$result_p1=mysql_query(" SELECT email FROM login WHERE  password LIKE '$password' ");
$row_p1=mysql_fetch_array($result_p1);
$email=$row_p1['email'];
$result_p2=mysql_query(" SELECT Bank_Name FROM blood_bank WHERE  Email LIKE '$email' ");
$row_p2=mysql_fetch_array($result_p2);
$bankname=$row_p2['Bank_Name'];




	$sele = "SELECT name,email,phone,user_type,message,q_date FROM user_query WHERE  bank_name LIKE '$bankname' order by q_date desc";
	...
	echo "<tr>";
     ...
     // echo "<td>";echo $row['phone'];echo "</td>";
      ...
      echo "<td>";echo $row['q_date'];echo "</td>";
     echo "</tr>";
	 echo "</tbody>";
		} 		
}
 ...
 $script = file_get_contents('stock_redirect.js');
