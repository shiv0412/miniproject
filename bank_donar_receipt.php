...
<body>
  ...
  <div class="Receipt_header" style="text-align:center; width:49%;margin:auto;border:1px solid black;">
    ...
    <h3>Bank Donar Receipt</h3>
    <p style="margin-top:10px;">Congratulations.You are a lifesaver.Reach your registered blood bank within a week to avail rush free donation.</p>
   </div></div>
  <div style="width:100%;">
...
$result = mysqli_query($con,"SELECT b_donar_id  FROM blood_bank_donar order by b_donar_id desc limit 1");
...
$g=$_POST["city"];
$h=$_POST["bloodbank"];
$sql = "INSERT INTO `digital_blood_bank`.`blood_bank_donar` (`name`, `mobile`, `gender`, `dob`, `bloodgroup`, `state`, `city`, `blood_bank`)
...
$result = mysqli_query($con,"SELECT b_donar_id,register_date,name,blood_bank,gender,bloodgroup  FROM blood_bank_donar where b_donar_id='$id'");
...
echo "<td class='receipt_bod'>". $row['b_donar_id'] . "</td>";
echo "</tr>";
echo "<tr>";
